/**
 * AI_CONTEXT: Auth Module — Magic link, JWT sessions, rate limiting.
 * No passwords. No cookies dependency. Pure functions + DB.
 *
 * Dependencies:
 *   - src/db.js (better-sqlite3 instance)
 *
 * Exports:
 *   - TIER_LIMITS, getTierConfig(tier)
 *   - parseCookies(req)
 *   - createAccount, getAccountByEmail, getAccountById, updateAccountTier, updateAccountStripe
 *   - createMagicLinkToken, verifyMagicLinkToken
 *   - createSession, verifySession, revokeSession
 *   - requireAuth(req)
 *   - checkRateLimit, logUsage, updateUsageTokens
 *   - checkTokenBudget, getUsageStats
 */
